...
def generate_random_split(image_dir,text_dir,folds,test_ratio,train_ratio):
  test = test_ratio;
  train = train_ratio;
  dev = 1 - test - train;
...
def generate_spin_split(image_dir, text_dir,folds):
  '''
...
def main(argv):
  ...
  try:
    ...
    print('Generate Random splits each time')
    r = 0;

  try:
    test_ratio = float(args['-e'])
  except KeyError:
    print('Using default test ratio of 0.1');
    test_ratio = 0.1;
   
  try:
    train_ratio = float(args['-a']);
  except KeyError:
    print('Using default train ratio of 0.8');
    train_ratio = 0.8;  

  if r == 1:
    dir = generate_spin_split(image_dir,text_dir,n)
  else:
    dir = generate_random_split(image_dir,text_dir,n,test_ratio,train_ratio)

class Arguments:

    options = ["-p", "-m", "-k", "-s", "-r", "-u", "-d", "-n", "-l", "-x", "-v", "-f", "-i","-t","-a","-e"] # -h is reserved.
...
def usage():
    ...
    print
    print("generateRandomSplit.py randomly generating data splits for tests")
...
#    print
    print
    print("Usage: python generateRandomSplit.py -s {Spin} -i {image dir} -t {text dir} -n {number of folds} -e {test data ratio} -a {train data ratio}")
