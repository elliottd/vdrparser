...
class POSTag:
  ...
  def tag(self):
      ...
      filelist = self.concatenate_text(directory, "desc", directory+"/descriptions")
      self.pos_tag(directory, "descriptions")
      files = sorted([x for x in os.listdir(directory) if x.endswith(".desc")])
      self.unfurl_tags(directory+"/descriptions-tagged", files, directory, filelist)
      print

  def concatenate_text(self, path, extension, output_file):
      '''
      Concatenate a collection of files with a given extension into
      the output_file.
      '''
      print("Concatenating the text files to speed up tagging: cat %s/*.%s ..." % (path, extension))
      files = os.listdir(path)
      files = sorted([x for x in files if x.endswith(extension)])
      tmp_strings = open("%s" % (output_file), "w")
      #filelist_fh = open("filelist", "w")
      filelist = []
      for x in files:
      ...
      #destination.close()
      print("... done. Grokked %d files" % len(files))
      print
...
class DependencyParse:
  ...
  def parse(self):
      ...
      files = sorted([x for x in os.listdir(directory) if x.endswith(".desc")])
      self.unfurl_parsed(directory+"/descriptions-tagged-conll-parsed", files, directory)
